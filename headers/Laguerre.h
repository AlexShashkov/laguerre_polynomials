...
private:
     ...
     */
    inline void laguer(const std::vector<std::complex<T>>& a, std::complex<T>& x, int& converged, int m, int itmax){
        converged = 1;
        ...
        for (int iter = 1; iter <= itmax; iter++) {
            ...
            x = iter % MT != 0 ? x1: fma(x, -frac[iter / MT], dx);
        }
        converged = -1;
